...
namespace BattleShips.Client
{
    public partial class Form1 : Form
    {
        ...
        public Form1()
        {
            ...
            string theme = random.Next(2) == 0 ? "dark" : "light";
            _renderer = new BoardRenderer(40, 80, theme);

            Console.WriteLine($"[Form1] Using {theme} theme");
        ...
        private void DisplayMotivationalMessage()
        {
            ...
            string previousMessage = _model.CurrentMotivationalMessage;
            _model.AdvanceMotivationalMessage();
            string newMessage = _model.CurrentMotivationalMessage;

            // Display in terminal (Console)
            Console.WriteLine($"==========================================");
            Console.WriteLine($"🎮 TURN MOTIVATIONAL MESSAGE:");
            Console.WriteLine($"Iterator: {_model.GetIteratorType()}");
            Console.WriteLine($"Status: {_model.GetIteratorStatus()}");
            Console.WriteLine($"Message: {newMessage}");
            Console.WriteLine($"==========================================");

            // Also display in game UI (optional)
            if (_lblStatus != null && !string.IsNullOrEmpty(newMessage))
            {
                _lblStatus.Text = newMessage;
            }

            Invalidate(); // Refresh UI if displaying message visually
        }

        // Call this method when turn changes in your game logic
        // For example, when opponent's turn ends and your turn begins
        ...
        private void DrawMotivationalMessage(Graphics g)
        {
            using var messageFont = new Font("Segoe UI", 10, FontStyle.Italic);
            using var messageBrush = new SolidBrush(Color.FromArgb(220, 220, 180));

            string message = _model.CurrentMotivationalMessage;
            string iteratorInfo = $"Iterator: {_model.GetIteratorType()} ({_model.GetIteratorStatus()})";

            var messageSize = g.MeasureString(message, messageFont);
            var infoSize = g.MeasureString(iteratorInfo, messageFont);

            // Position at top center
            float x = (ClientSize.Width - messageSize.Width) / 2;
            float infoX = (ClientSize.Width - infoSize.Width) / 2;

            // Draw message
            g.DrawString(message, messageFont, messageBrush, x, 30);

            // Draw iterator info in smaller font
            using var infoFont = new Font("Segoe UI", 8, FontStyle.Regular);
            using var infoBrush = new SolidBrush(Color.FromArgb(180, 180, 180));
            g.DrawString(iteratorInfo, infoFont, infoBrush, infoX, 55);
        }
