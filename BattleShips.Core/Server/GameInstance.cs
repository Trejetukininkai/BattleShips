...
namespace BattleShips.Core
{
    ...
    public class GameInstance
    {
        ...
        public List<ShipBlock> ShipsA { get; set; } = new();
        public List<ShipBlock> ShipsB { get; set; } = new();
        ...
        /// <summary>
        /// Converts placed ship cells into ShipBlock objects using composite pattern
        /// </summary>
        /// <summary>
        /// Converts placed ship cells into ShipBlock objects using composite pattern
        /// </summary>
        public void SetPlayerShips(string connId, List<Point> shipCells)
        {
            ...
            while (placedCells.Any())
            {
                ...
                // Create ShipBlock with ShipCell children
                var shipBlock = ShipBlock.CreateFromPoints(horizontalCells);

                // Set additional properties for compatibility
                shipBlock.Class = (connId == PlayerA ? ClassA : ClassB) ?? ShipClass.Blocky;
                shipBlock.ShipType = DetermineShipTypeFromSize(horizontalCells.Count);
        ...
        private string DetermineShipTypeFromSize(int size)
        {
            return size switch
            {
                5 => "AircraftCarrier",
        ...
        public bool RegisterShot(string opponentConnId, Point shot, out bool opponentLost)
        {
            ...
            // Check if shot hits any ship using composite pattern
            foreach (var ship in ships)
            {
                if (ship.GetOccupiedCells().Contains(shot))
                {
                    hit = true;
                    // Find and hit the specific ship cell
                    foreach (var component in ship.GetChildren())
                    {
                        if (component is ShipCell cell && cell.Position.Equals(shot))
                        {
                            cell.Hit();
                            break;
                        }
            ...
            opponentLost = ships.All(s => s.IsSunk());
        ...
        public void RegisterDisasterHits(string playerConnId, List<Point> hitPoints)
        {
            ...
            var ships = PlayerA == playerConnId ? ShipsA : ShipsB;

            foreach (var point in hitPoints)
            {
                hitCells.Add(point);

                // Apply hit to ships if they're at that position
                foreach (var ship in ships)
                {
                    foreach (var component in ship.GetChildren())
                    {
                        if (component is ShipCell cell && cell.Position.Equals(point))
                        {
                            cell.Hit();
                            break;
                        }
        ...
        public bool RegisterShotWithMines(string opponentConnId, Point shot, out bool opponentLost, out List<(Guid mineId, MineCategory category, List<Point> effectPoints)> triggeredMines)
        {
            ...
            opponentLost = opponentShips.All(s => s.IsSunk());
        ...
        public void DebugShipPositions()
        {
            ...
            foreach (var ship in ShipsA)
            {
                var cells = ship.GetOccupiedCells();
                Console.WriteLine($"[GameInstance]   - Ship {ship.Name} ({cells.Count} cells): {string.Join(", ", cells.Select(p => $"({p.X},{p.Y})"))}");
                Console.WriteLine($"[GameInstance]     Status: {(ship.IsSunk() ? "SUNK" : "ACTIVE")}");
            }

            Console.WriteLine($"[GameInstance] 🚢 PlayerB ships:");
            foreach (var ship in ShipsB)
            {
                var cells = ship.GetOccupiedCells();
                Console.WriteLine($"[GameInstance]   - Ship {ship.Name} ({cells.Count} cells): {string.Join(", ", cells.Select(p => $"({p.X},{p.Y})"))}");
                Console.WriteLine($"[GameInstance]     Status: {(ship.IsSunk() ? "SUNK" : "ACTIVE")}");
        ...
        public void ApplyDisasterWithMines(string playerConnId, List<Point> hitPoints, out List<(Guid mineId, MineCategory category, List<Point> effectPoints)> triggeredMines)
        {
            ...
            var playerShips = PlayerA == playerConnId ? ShipsA : ShipsB;
            ...
            // Log all disaster hit points and apply to ships
            foreach (var p in hitPoints)
            {
                ...
                foreach (var ship in playerShips)
                {
                    foreach (var component in ship.GetChildren())
                    {
                        if (component is ShipCell cell && cell.Position.Equals(p))
                        {
                            cell.Hit();
                            break;
                        }
        ...
        // ========================================
        // COMPOSITE PATTERN HELPER METHODS
        // ========================================

        /// <summary>
        /// Creates a test ship using composite pattern (for testing)
        /// </summary>
        public ShipBlock CreateTestShip(Point startPosition, bool isHorizontal, int size = 4)
        {
            return ShipBlock.CreateShip(size, startPosition, isHorizontal);
        }

        /// <summary>
        /// Gets all occupied cells (for debugging or validation)
        /// </summary>
        public List<Point> GetAllOccupiedCells(string playerConnId)
        {
            var ships = PlayerA == playerConnId ? ShipsA : ShipsB;
            ...
            foreach (var ship in ships)
            {
                occupied.AddRange(ship.GetOccupiedCells());
            }
        ...
        /// <summary>
        /// Process a shot on specific ship cell (for testing)
        /// </summary>
        public string ProcessShot(string playerConnId, Point target)
        {
            var ships = PlayerA == playerConnId ? ShipsA : ShipsB;
            ...
            foreach (var ship in ships)
            {
                foreach (var component in ship.GetChildren())
                {
                    if (component is ShipCell cell && cell.Position.Equals(target))
                    {
                        if (cell.IsHit)
                            return "AlreadyShot";
        ...
        public GameMemento CreateMemento()
        {
            var memento = new GameMemento(Id)
            {
                ...
                IsPlayerATurn = CurrentTurn == PlayerA,
                TurnCount = TurnCount,
            ...
            // We cannot save ShipBlock directly to ShipData because they have different structures
            // For now, we'll skip saving ships in memento for composite pattern
            // This is a temporary workaround
            memento.SetShipsAData(new List<ShipData>());
            memento.SetShipsBData(new List<ShipData>());

            memento.SetHitCellsA(HitCellsA.ToList());
            memento.SetHitCellsB(HitCellsB.ToList());

            // Skip mines for now since NavalMine is abstract
            memento.SetMinesAData(new List<MineData>());
            memento.SetMinesBData(new List<MineData>());

            Console.WriteLine($"[GameInstance] Created memento for game {Id} (Note: Ship data not saved for composite pattern)");
            return memento;
        }
        ...
        public void RestoreFromMemento(GameMemento memento)
        {
            ...
            // Note: We cannot restore ShipBlock from ShipData because they have different structures
            // For composite pattern, we'll leave ships empty
            ShipsA = new List<ShipBlock>();
            ShipsB = new List<ShipBlock>();

            // Restore hit cells
            HitCellsA = new HashSet<Point>(memento.HitCellsA);
            HitCellsB = new HashSet<Point>(memento.HitCellsB);

            // Skip mines restoration (NavalMine is abstract)
            MinesA = new List<NavalMine>();
            MinesB = new List<NavalMine>();
            ...
            Console.WriteLine($"[GameInstance] Note: Ships not restored due to composite pattern incompatibility");
        }
    }
}